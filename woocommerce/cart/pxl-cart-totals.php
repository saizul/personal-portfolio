...
$cart_payment_methods_text = foliohub()->get_theme_opt('cart_payment_methods_text','');
$cart_payment_methods_logo = foliohub()->get_theme_opt('cart_payment_methods_logo',[]);
...
<div class="cart_totals <?php echo ( WC()->customer->has_calculated_shipping() ) ? 'calculated_shipping' : ''; ?>">
	...
	<h4 class="pxl-heading"><?php esc_html_e( 'Cart Totals', 'foliohub' ); ?></h4>

	<div class="cart-total-inner">
		<div class="sub-total cart-total-row d-flex align-items-center justify-content-between">
			<span class="lbl"><?php esc_html_e( 'Subtotal', 'foliohub' ); ?></span>
		...
		<?php elseif ( WC()->cart->needs_shipping() && 'yes' === get_option( 'woocommerce_enable_shipping_calc' ) ) : ?>
			...
			<div class="shipping cart-total-row d-flex align-items-center justify-content-between"> 
				<span class="lbl"><?php esc_html_e( 'Shipping', 'foliohub' ); ?></span>
		...
		if ( wc_tax_enabled() && ! WC()->cart->display_prices_including_tax() ) {
			...
			if ( WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping() ) { 
				$estimated_text = sprintf( ' <small>' . esc_html__( '(estimated for %s)', 'foliohub' ) . '</small>', WC()->countries->estimated_for_prefix( $taxable_address[0] ) . WC()->countries->countries[ $taxable_address[0] ] );
			}
		...
		<div class="order-total d-flex align-items-center justify-content-between">
			<span class="lbl"><?php esc_html_e( 'Total', 'foliohub' ); ?></span>
			<span class="value"><?php wc_cart_totals_order_total_html(); ?></span>
		</div>
		<p class="order-taxes"><?php esc_html_e( 'Taxes and shipping calculated at checkout', 'foliohub' ); ?></p>
