...
class Foliohub_CSS_Generator {
	...
	function __construct() {
		$this->opt_name = foliohub()->get_option_name();  
        ...
        add_action( 'wp_enqueue_scripts', array( $this, 'foliohub_enqueue' ), 20 );
	}
	...
	function init() {
		...
		add_action( 'wp', array( $this, 'foliohub_generate_with_dev_mode' ) );
		add_action( "redux/options/{$this->opt_name}/saved", function () {
			$this->foliohub_generate_file_options();
		} );
	}

	function foliohub_generate_with_dev_mode() {
		if ( $this->dev_mode === true ) {
            $this->foliohub_generate_file_options();
			$this->foliohub_generate_file();
		}
	}

    function foliohub_generate_file_options() {
        ...
        $this->redux->filesystem->execute( 'put_contents', $_options, array(
            'content' => preg_replace( "/(?<=[^\r]|^)\n/", "\r\n", $this->foliohub_options_output() )
        ) );
    }

	function foliohub_generate_file() {
		$scss_dir = get_template_directory() . '/assets/scss/';
	...
	protected function foliohub_options_output() {
		$theme_colors                    = foliohub_configs('theme_colors');
        $links                           = foliohub_configs('link');
        $gradients                       = foliohub_configs('gradient');
        $gradient_color_two        = foliohub_configs('gradient_two');
		ob_start();

		printf('$foliohub_theme_colors_key:(%s);',$this->print_scss_opt_colors($theme_colors,'key'));
        printf('$foliohub_theme_colors_val:(%s);',$this->print_scss_opt_colors($theme_colors,'val'));
        // color rgb only
    ...
    /* Inline CSS */
    function foliohub_enqueue() {
        $css = $this->foliohub_inline_css();
        if ( !empty( $css ) ) {
            wp_add_inline_style( 'pxl-style', $css );
        }
    }
    protected function foliohub_inline_css() {
        ob_start();
