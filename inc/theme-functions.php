 ...
 * Google Fonts
*/
function foliohub_fonts_url() {
    $fonts_url = '';
    $fonts     = array();
    $subsets   = 'latin,latin-ext';
    
    if ( 'off' !== _x( 'on', 'Anton font: on or off', 'foliohub' ) ) {
        $fonts[] = 'Anton&display=swap';
    }

    if ( 'off' !== _x( 'on', 'Inter font: on or off', 'foliohub' ) ) {
 ...
 * Get page ID by Slug
*/
function foliohub_get_id_by_slug($slug, $post_type){
 ...
 **/
function foliohub_content_by_slug($slug, $post_type){
    $content = foliohub_get_content_by_slug($slug, $post_type);

    $id = foliohub_get_id_by_slug($slug, $post_type);
 ...
 * Get content by slug
 **/
function foliohub_get_content_by_slug($slug, $post_type){
    $content = get_posts(
        array(
            'name'      => $slug,
            'post_type' => $post_type
 ...
 * Custom Comment List
 */
function foliohub_comment_list( $comment, $args, $depth ) {
    if ( 'div' === $args['style'] ) {
        $tag       = 'div';
        $add_below = 'comment';
    } else {
            ...
            <div class="comment-content">
                <div class="comment-holder pxl-flex">
                    <?php if ($args['avatar_size'] != 0) : ?> 
                        <div class="comment-image pxl-mr-20">
 ...
 * Paginate Links
 */
function foliohub_ajax_paginate_links($link){
 ...
 * RGB Color
 */
function foliohub_hex_rgb($color) {
...
///////
function foliohub_set_post_views($postID) {
...
function foliohub_track_post_views() {
    if ( is_single() ) {
        global $post;
        if ( $post ) {
            foliohub_set_post_views( $post->ID );
        }
    }
}
add_action( 'wp_head', 'foliohub_track_post_views' );
function foliohub_get_post_views( $postID ) {
 ...
 * Image Size Crop
 */
if(!function_exists('foliohub_get_image_by_size')){
    function foliohub_get_image_by_size( $params = array() ) {
 ...
 * Search Form
 */
function foliohub_header_mobile_search_form() { 
    $search_mobile = foliohub()->get_theme_opt( 'search_mobile', false );
    $search_placeholder_mobile = foliohub()->get_theme_opt( 'search_placeholder_mobile' );
    if($search_mobile) : ?>
        <div class="pxl-header-mobile-search pxl-hide-xl">
            <form role="search" method="get" action="<?php echo esc_url(home_url( '/' )); ?>">
                <input type="text" placeholder="<?php if(!empty($search_placeholder_mobile)) { echo esc_attr($search_placeholder_mobile); } else { esc_attr_e('Search...', 'foliohub'); } ?>" name="s" class="search-field" />
...
if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_year_shortcode() {
        ob_start(); ?>
        <span><?php the_date('Y'); ?></span>
        <?php $output = ob_get_clean();
        return $output;
    }
    pxl_register_shortcode('pxl_year', 'foliohub_year_shortcode');
}

/* Highlight Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_text_highlight_shortcode( $atts = array() ) {
    ...
    pxl_register_shortcode('highlight', 'foliohub_text_highlight_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_text_highlight_shortcode_editor( $atts = array() ) {
        extract(shortcode_atts(array(
           'text' => '',
       ), $atts));
    ...
    pxl_register_shortcode('pxl_highlight', 'foliohub_text_highlight_shortcode_editor');
}

/* Typewriter Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_text_typewriter_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'text' => '',
       ), $atts));
    ...
    pxl_register_shortcode('typewriter', 'foliohub_text_typewriter_shortcode');
}

/* Button Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_btn_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'text' => '',
           'style' => '',
           'icon_class' => '',
    ...
    pxl_register_shortcode('pxl_button', 'foliohub_btn_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_btn_video_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'text' => '',
           'url' => '',
       ), $atts));
    ...
    pxl_register_shortcode('pxl_btn_video', 'foliohub_btn_video_shortcode');
}


if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_btn_submit_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'text' => '',
    ...
    pxl_register_shortcode('pxl_button_submit', 'foliohub_btn_submit_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_slider_arrow( $atts = array() ) {
        extract(shortcode_atts(array(
           'type' => 'next',
           'style' => 'style-1',
       ), $atts));
...
pxl_register_shortcode('slider_arrow', 'foliohub_slider_arrow');
}

// Shortcode Row/Column Grid
if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_start_row_shortcode( $atts = array() ) {
        ob_start(); ?>
        <div class="pxl-post-container">
            ...
            pxl_register_shortcode('pxl_start_row', 'foliohub_start_row_shortcode');
        }

        if(function_exists( 'pxl_register_shortcode' )) {
            function foliohub_end_row_shortcode( $atts = array() ) {
                ob_start(); ?>
            </div>
        </div>       
        <?php $output = ob_get_clean();
    ...
    pxl_register_shortcode('pxl_end_row', 'foliohub_end_row_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_start_col_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'class' => 'col-12',
       ), $atts));
       ob_start(); ?>
    ...
    pxl_register_shortcode('pxl_start_column', 'foliohub_start_col_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_end_col_shortcode( $atts = array() ) {
        ob_start(); ?>
    </div>  
    <?php $output = ob_get_clean();
    return $output;
}
pxl_register_shortcode('pxl_end_column', 'foliohub_end_col_shortcode');
}
...
/* Gallery Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_gallery_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'link_video' => '',
           'images_id' => '',
           'col' => '2',
            ...
            foreach ($pxl_images as $key => $img_id) :
                ...
                <div class="pxl--item">
                    <div class="pxl--item-inner">
                        <a href="<?php echo esc_url($thumbnail_url); ?>" data-elementor-lightbox-slideshow="pxl-gallery-<?php echo esc_attr($pxl_g_id); ?>"><?php echo foliohub_html($thumbnail); ?></a>
    ...
    pxl_register_shortcode('pxl_gallery', 'foliohub_gallery_shortcode');
}

/* Addd shortcode Video button */
if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_video_button_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'link' => '',
           'text' => '',
           'class' => 'style1',
    ...
    pxl_register_shortcode('pxl_video_button', 'foliohub_video_button_shortcode');
}

/* Get Category Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_post_category_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'items' => '6',
           'columns' => '2',
       ), $atts));
    ...
    pxl_register_shortcode('pxl_post_category', 'foliohub_post_category_shortcode');
}

/* Slider 1  */
if(function_exists( 'pxl_register_shortcode' )) {
    function foliohub_slider_price_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'price' => '',
           'desc' => '',
       ), $atts));
    ...
    pxl_register_shortcode('pxl_slider_price', 'foliohub_slider_price_shortcode');
}

/**
 * Custom Widget Categories - Count
 */
add_filter('wp_list_categories', 'foliohub_wg_category_count');
function foliohub_wg_category_count($output) {
 ...
 */
add_filter('get_archives_link', 'foliohub_wg_archive_count');
function foliohub_wg_archive_count($links) {
 ...
 */
add_filter('wp_list_categories', 'foliohub_wc_cat_count_span');
function foliohub_wc_cat_count_span($links) {
    $dir = is_rtl() ? 'pxl-left' : 'pxl-right';
 ...
 */
function foliohub_get_mega_menu_builder_id(){
    $mn_id = [];
 ...
 */
function foliohub_get_page_popup_builder_id(){
    $pp_id = [];
...
function foliohub_mouse_move_animation() { 
    $mouse_move_animation = foliohub()->get_theme_opt('mouse_move_animation', false); 
    if($mouse_move_animation) {
        wp_enqueue_script( 'foliohub-cursor', get_template_directory_uri() . '/assets/js/libs/cursor.js', array( 'jquery' ), '1.0.0', true ); ?>  
 ...
 * Start - Cookie Policy
 */
function foliohub_cookie_policy() {
    $cookie_policy = foliohub()->get_theme_opt('cookie_policy', 'hide');
    $cookie_policy_description = foliohub()->get_theme_opt('cookie_policy_description');
    $cookie_policy_btntext = foliohub()->get_theme_opt('cookie_policy_btntext');
    $cookie_policy_link = get_permalink(foliohub()->get_theme_opt('cookie_policy_link')); 
 ...
 */
function foliohub_subscribe_popup() {
    $subscribe = foliohub()->get_theme_opt('subscribe', 'hide');
    $subscribe_layout = foliohub()->get_theme_opt('subscribe_layout');
    $popup_effect = foliohub()->get_theme_opt('popup_effect', 'fade');
    $args = [
 ...
 */
add_action( 'show_user_profile', 'foliohub_user_fields' );
add_action( 'edit_user_profile', 'foliohub_user_fields' );
function foliohub_user_fields($user){
    ...
    ?>
    <h3><?php esc_html_e('Theme Custom', 'foliohub'); ?></h3>
    <table class="form-table">
        <tr>
            <th><label for="user_name"><?php esc_html_e('Author Name', 'foliohub'); ?></label></th>
            <td>
        ...
        <tr>
            <th><label for="user_position"><?php esc_html_e('Author Position', 'foliohub'); ?></label></th>
            <td>
...
function foliohub_get_user_name() {
...
function foliohub_get_user_position() {
...
function foliohub_attachment_field_credit( $form_fields, $post ) {
...
add_filter( 'attachment_fields_to_edit', 'foliohub_attachment_field_credit', 1, 2 );


function foliohub_attachment_field_credit_save( $post, $attachment ) {
...
add_filter( 'attachment_fields_to_save', 'foliohub_attachment_field_credit_save', 1, 2 );
/* End Custom Field Media */

/* Author Social */
function foliohub_get_user_social() {
